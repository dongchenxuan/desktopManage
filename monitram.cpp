...
monitRAM::monitRAM(QWidget *parent) : QWidget(parent)
{
    m_iBorderWidth = 0;
...
QPoint monitRAM::getQScreenSize(){
...
void monitRAM::paintEvent(QPaintEvent *event)
{
    //进度条不可见的情况下就不重绘了
    if (!this->isVisible())
    {
    ...
    m_iValue = get_MemUsage();//this->value() < 0 ? 0 : this->value();//初始化时QT进度条的进度为-1，避免负进度的出现
...
void monitRAM::drawBackGround(QPainter* painter)
{
...
void monitRAM::drawWaterWave(QPainter* painter)
{
    int width = this->width();
    int height = this->height();
...
void monitRAM::drawText(QPainter* painter)
{
    painter->save();
    int width = this->width();
    int height = this->height();
    ...
    painter->drawText(QRectF(m_iBorderWidth, m_iBorderWidth, diameter, diameter), Qt::AlignCenter, QString("RAM%1%").arg(m_iValue));
    painter->restore();
}
